...
def update_show_episode_counts(db: Session, show_id: int):
    """
    Update denormalised episode counts for a show.
    Counts total episodes and watched episodes.
    """
    show = db.query(TrackedShow).filter(TrackedShow.tmdb_show_id == show_id).first()
    if show:
...
async def get_tracked_shows(
    ...
    """
    Fetch all tracked TV shows with optional filters.
    Used by library screen to display user's TV collection.
    """
    query = db.query(TrackedShow)
    ...
    query = query.order_by(TrackedShow.updated_at.desc())
    shows = query.all()

    return [
        ShowTrackingResponse(
...
async def get_tracked_show(tmdb_id: int, db: Session = Depends(get_db)):
    """
    Fetch tracking data for a specific TV show.
    Returns favourited status, rating, comment, and watch progress.
    Returns None if show is not tracked.
    """
...
async def track_show(
    ...
    """
    Add tracking data for a TV show.
    Creates new record if show not tracked, updates existing otherwise.
    Fetches total episode count from TMDB if not already set.
    """
...
async def update_show_tracking(
    ...
    Returns 404 if show is not currently tracked.
    """
    show = db.query(TrackedShow).filter(
        TrackedShow.tmdb_show_id == tmdb_id
    ).first()
    ...
    if not show:
        raise HTTPException(
            status_code=404,
            detail=f"TV show with TMDB ID {tmdb_id} is not tracked"
        )
...
async def untrack_show(tmdb_id: int, db: Session = Depends(get_db)):
    """
    Remove a TV show from tracked collection.
    ...
    return {
        "message": "TV show untracked successfully",
        "tmdb_show_id": tmdb_id
    }
...
async def get_show_episodes(tmdb_id: int, db: Session = Depends(get_db)):
    """
    Get all tracked episode watch states for a show.
...
async def mark_episodes(
    ...
    """
    Mark multiple episodes as watched or unwatched.
    ...
    if not show:
        raise HTTPException(
            status_code=404,
            detail=f"TV show with TMDB ID {tmdb_id} is not tracked. Track the show first."
        )

    updated_episodes = []

    for ep_request in request.episodes:
        # Check if episode already tracked
    ...
    update_show_episode_counts(db, tmdb_id)
...
async def get_show_progress(tmdb_id: int, db: Session = Depends(get_db)):
    """
